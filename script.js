...
mapboxgl.accessToken =
  "pk.eyJ1IjoiaGVtYW50aDA0IiwiYSI6ImNtazZtaGswNzAyOWQzZXNkanhyaTlkY3UifQ.Ofpbw_IWXGXdG-PgUphK0g";
...
function setupEventListeners() {
  ...
  document
    .getElementById("toggleOnline")
    .addEventListener("click", toggleOnlineStatus);
  ...
  document
    ...
    .addEventListener("click", completeRide);

  // Reassignment buttons
  document
    .getElementById("reassignRequest")
    .addEventListener("click", showNearbyDrivers);
  document
    .getElementById("reassignActiveRide")
    .addEventListener("click", openEmergencyModal);

  // Emergency buttons
  document
    .getElementById("raiseEmergency")
    .addEventListener("click", openEmergencyModal);
  ...
  document
    ...
    .addEventListener("click", submitEmergencyRequest);
...
function toggleOnlineStatus() {
  ...
  if (isOnline) {
    // Go online - Register driver with backend
    registerOrUpdateDriver("AVAILABLE").then((result) => {
      if (result) {
        console.log("[UI] Driver registered with backend");
        addNotification(
          "system",
          "Backend Connected",
          "Your driver profile has been synced with the system.",
          "success"
        );
      }
    });
    ...
    addNotification(
      "system",
      "You are now online",
      "You will start receiving ride requests shortly.",
      "info"
    );
  } else {
    // Go offline
    registerOrUpdateDriver("AVAILABLE").then((result) => {
      if (result) {
        console.log("[UI] Driver status updated to offline");
      }
    });
    ...
    statusBarText.textContent =
      'You\'re currently offline. Tap "Go Online" to start receiving rides.';
...
function simulateRideRequest() {
  ...
  const pickupLocations = [
    {
      ...
      coords: [77.6105, 12.9758],
      address: "Near UB City",
    },
    {
      name: "Indiranagar, Bangalore",
      coords: [77.6408, 12.9784],
      address: "100 Feet Road",
    },
    {
      name: "Koramangala, Bangalore",
      coords: [77.6245, 12.9279],
      address: "Near Forum Mall",
    },
    {
      name: "Whitefield, Bangalore",
      coords: [77.75, 12.9698],
      address: "ITPL Road",
    },
  ];
  ...
  const dropLocations = [
    {
      ...
      coords: [77.5827, 12.9308],
      address: "4th Block",
    },
    {
      name: "HSR Layout, Bangalore",
      coords: [77.6387, 12.9123],
      address: "Sector 7",
    },
    {
      name: "Malleswaram, Bangalore",
      coords: [77.5667, 13.0067],
      address: "8th Cross",
    },
    {
      name: "Yeshwanthpur, Bangalore",
      coords: [77.5393, 13.0232],
  ...
  document.getElementById(
    "statusText"
  ).textContent = `New ride request received! You have 30 seconds to respond.`;
}
...
function startRequestTimer() {
  ...
  requestTimer = setInterval(() => {
    ...
    if (requestTimeLeft <= 10) {
      document.querySelector(".request-timer").style.backgroundColor =
        "#f44336";
    }
...
function autoRejectRide() {
  ...
  document.getElementById(
    "statusText"
  ).textContent = `Ride auto-rejected due to timeout. Waiting for new requests.`;
...
async function acceptRide() {
  ...
  const orderCreated = await createOrder(orderId, driverId);

  if (orderCreated) {
    console.log("[UI] Order created in backend");
  } else {
    console.warn("[UI] Order creation failed, continuing in local mode");
  }
  ...
  document.getElementById(
    "statusText"
  ).textContent = `Ride accepted! Heading to pickup at ${currentRide.pickup.name}`;

  // Set directions to pickup location
  directions.setOrigin([
    currentRide.pickup.coords[0],
    currentRide.pickup.coords[1],
  ]);
  directions.setDestination([
    currentRide.drop.coords[0],
    currentRide.drop.coords[1],
  ]);
...
function rejectRide() {
  ...
  document.getElementById(
    "statusText"
  ).textContent = `Ride rejected. Waiting for new requests.`;
...
function completeRide() {
  ...
  document.getElementById(
    "statusText"
  ).textContent = `Ride ${currentRide.id} completed! ₹${currentRide.fare} added to your earnings.`;
...
async function submitEmergencyRequest() {
  ...
  try {
    // Send delay event to backend
    let orderId = currentRide?.id || `ORDER-${Date.now()}`;
    let driverId = driverProfile.id;

    // Ensure order exists in backend
    if (currentRide && !currentRide.backedUp) {
      await createOrder(orderId, driverId);
      currentRide.backedUp = true;
    }

    // Report the delay event to backend
    const result = await reportDelayEvent(
      orderId,
      driverId,
    ...
    if (result) {
      ...
      const action = result.action_taken || "PROCESSED";
      ...
      statusDiv.innerHTML = `
                ...
                <p>Our system is processing your request. You will be notified shortly.</p>
            `;
      ...
      addNotification(
        ...
        `Your ${selectedPriority} priority reassignment request has been submitted. Risk Score: ${riskScore}%`,
        "emergency"
      );

      // If reassignment was initiated, start the process
      if (
      ...
      statusDiv.innerHTML = `
                ...
                <p>Error: Backend communication failed. Please check your connection.</p>
            `;
    }
  } catch (error) {
    console.error("[UI] Error submitting emergency request:", error);
    statusDiv.className = "reassignment-status status-failed active";
    statusDiv.innerHTML = `
            <h4><i class="fas fa-exclamation-circle"></i> Request Failed</h4>
            <p>An error occurred: ${error.message}</p>
            <p>Please try again or call support.</p>
        `;
  }
}
...
function startReassignmentProcess() {
  ...
  document.getElementById(
    "statusText"
  ).textContent = `Looking for alternative drivers for ride ${currentRide.id}...`;
...
function assignRideToDriver(driverId) {
  ...
  const driverName = document
    .querySelector(`[data-driver-id="${driverId}"]`)
    .closest(".driver-item")
  ...
  document.getElementById(
    "statusText"
  ).textContent = `Requesting ${driverName} to accept ride ${currentRide.id}...`;
  ...
  setTimeout(() => {
    ...
    if (success) {
      ...
      document.getElementById(
        "statusText"
      ).textContent = `${driverName} accepted ride ${currentRide.id}. Reassignment successful!`;
      ...
      document.getElementById(
        "statusText"
      ).textContent = `${driverName} declined the ride. Looking for other drivers...`;
...
function completeReassignment() {
  ...
  setTimeout(() => {
    ...
    document.getElementById(
      "statusText"
    ).textContent = `Ride reassigned successfully. You are now available for new rides.`;
...
function simulateDriverAssignments() {
  ...
  const attemptAssignment = () => {
    ...
    setTimeout(() => {
      if (Math.random() > 0.6 && reassignmentInProgress) {
        // 40% success rate
        // Successfully assigned
        document.getElementById(
          "statusText"
        ).textContent = `Ride ${currentRide.id} has been reassigned to another driver.`;

        addNotification(
          "reassignment_auto",
          "Ride Auto-Reassigned",
        ...
        // Try again
        document.getElementById("statusText").textContent = `Attempt ${
          attempts + 1
        ...
        document.getElementById(
          "statusText"
        ).textContent = `Could not reassign ride ${currentRide.id}. Please continue the ride or call support.`;
...
function initializeNotifications() {
  ...
  addNotification(
    "system",
    "Welcome to custom Driver",
    "You can go online to start receiving ride requests.",
    "info"
  );
  addNotification(
    "system",
    "Emergency Feature",
    "Use the emergency button to request reassignment in case of issues.",
    "info"
  );
}
